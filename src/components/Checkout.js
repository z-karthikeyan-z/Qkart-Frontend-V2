  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button
          variant="text"
          onClick = {()=> handleNewAddress({value:"", isAddingNewAddress: false})}
        >
          Cancel
        </Button>
      </Stack>
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const response = await axios.post(`${config.endpoint}/user/addresses`, {address:newAddress}, {headers:{Authorization: "Bearer "+token, 'Content-Type':'application/json'}});
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      const response = await axios.delete(`${config.endpoint}/user/addresses/${addressId}`, {headers:{Authorization: "Bearer "+token}});
      setAddresses({ ...addresses, all: response.data });
      return response.data;
            ...
            <Box>
              ...
              {addresses.all.length ? (
                addresses.all.map((add)=> (
                    <Box key={add._id} className = {
                      addresses.selected === add._id ? "address-item selected":"address-item not-selected"
                    }
                    onClick = {()=>{setAddresses({...addresses, selected: add._id})}}
                    >
                      <Typography color="#3C3C3C" my="1rem">{add.address}</Typography>
                      <Button
                      startIcon={<Delete />}
